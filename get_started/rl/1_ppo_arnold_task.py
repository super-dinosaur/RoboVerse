...
args = tyro.cli(Args)   #execute ScenarioCfg()
# load 'task' as ReachFarAwayCfg
# has 'traj_filepath' as the init file which is a dict, containing robot:list[init_state,...]
# param 'init_state' contains segments of robot
# param 'robot' contains 'dof' for each joints and three dimensions 'pos', four dimensions 'rot'
...
def train_ppo():
    ...
    if hasattr(args,'objects') and args.objects:
        scenario.objects = args.objects
        print(f"Added {len(scenario.objects)} objects to scenario")
    scenario.objects = [
        PrimitiveCubeCfg(
            name="cube",
            size=(0.1, 0.1, 0.1),
    ...
    print(f"Scenario objects: {scenario.objects}")
    scenario.task = None
