...
def get_task(task_id: str) -> BaseTaskCfg:
    ...
    cprint(f"import_path:{import_path}",color='red',attrs=["bold"])

    #use import_module to import the config file
    module = importlib.import_module(import_path)
    #use getattr() to get its specific class
    task_cls = getattr(module, f"{task_name_camel}Cfg")
    print(f"{task_name_camel}Cfg")
