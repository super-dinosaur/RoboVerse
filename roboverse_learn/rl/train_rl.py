...
#search the config files under roboverse_learn/rl/configs/default.yaml
@hydra.main(config_path="configs/train", config_name="myAllegroHandReorientationPPO")
def main(cfg: DictConfig):
    cprint("In @hydra.main.main()",color='red',attrs=["bold"])
    sim_name = cfg.environment.sim_name.lower()
    set_np_formatting()
    if sim_name == "isaacgym":
        import isaacgym  # noqa: F401

    cprint("before import",color='red',attrs=["bold"])
    ...
    cprint("after import",color='red',attrs=["bold"])

    cprint("Start Building the Environment", "green", attrs=["bold"])
    cprint(f"Additional,cfg: {cfg}",color='red',attrs=["bold"])
